...
class ExecuteCommandTool(Tool):
    ...
    def execute(self, command: str, working_dir: str = None) -> str:
        ...
        safe_commands_prefixes = [
            ...
            'tar -t', 'tar -tf', 'tar -tvf',
            'unzip -l', 'unzip -lv',
            'zip -sf',

            # Git (read-only)
            'git status', 'git log', 'git diff', 'git branch',
            'git show', 'git ls-files', 'git ls-tree',
            'git reflog', 'git remote', 'git tag',
        ...
        is_safe_command = any(command.strip().startswith(prefix) for prefix in safe_commands_prefixes)

        # Only ask for confirmation if required AND command is not safe
        if self.require_confirmation and self.get_confirmation_callback and not is_safe_command:
